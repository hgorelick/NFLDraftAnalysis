    ...
    for k, learner in enumerate(results.keys()):
        for j, metric in enumerate(['train_time', 'acc_train', 'f_train', 'pred_time', 'acc_test', 'f_test']):
            for i in np.arange(3):
                # Creative plot code
                ax[j // 3, j % 3].bar(i + k * bar_width, results[learner][metric], width=bar_width, color=colors[k])
...
def feature_plot(importances, X_train, y_train, num_arange):
    # if num_arange > 9:
    #     num_arange = 11
    # Display the five most important features
    indices = np.argsort(importances)[::-1]
    columns = X_train.columns.values[indices[:num_arange]]
    values = importances[indices][:num_arange]
    ...
    plt.bar(np.arange(num_arange), values, width=0.2, align="center", label="Feature Weight")
    # plt.bar(np.arange(11) - 0.3, np.cumsum(values), width = 0.2, align = "center", color = '#00A0A0', \
    #       label = "Cumulative Feature Weight")
    plt.xticks(np.arange(num_arange), columns)
    plt.xlim((-0.5, 4.5))
